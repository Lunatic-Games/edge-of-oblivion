...
func _ready():
	var rand_x_offset = randf_range(-50, 50)
	
	var transform_tween = get_tree().create_tween().set_parallel()
	transform_tween.tween_property(self, "global_position", global_position, 0.2).from(global_position + Vector2(rand_x_offset, -120)).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN)
