...
func trigger(starting_tile: Tile):
	...
	if tile_to_target and tile_to_target.occupant and tile_to_target.occupant == GameManager.player:
		var tile_coords: Vector2 = tile_to_target.get_2d_distance_to_tile(starting_tile)
...
func player_based_targeting(starting_tile: Tile) -> Tile:
	...
	var tile_size: float = current_tile.TILE_SIZE
	
	if abs(tile_coords.x) >= abs(tile_coords.y):
		if tile_coords.x > 0:
			for x in min(max_range, abs(tile_coords.x / tile_size)):
		...
		if tile_coords.y > 0:
			for x in min(max_range, abs(tile_coords.y / tile_size)):
